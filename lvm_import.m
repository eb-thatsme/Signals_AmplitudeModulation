function [x,fs] = lvm_import(filename)
data = lvm_import_base(filename,0);
t       = data.Segment1.data(:,1);
fs      = 1 / ( t(2)-t(1) );
x(:,1) = data.Segment1.data(:,2);
if data.Segment1.num_channels == 2
    x(:,2) = data.Segment1.data(:,4);
end
end

function data = lvm_import_base(filename,verbose)
